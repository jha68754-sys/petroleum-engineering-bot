...
def classify_fluid(gor: float, api: float) -> FluidClassification:
    """
...
def _validate_density_trend(
    ...
    """Validate oil density. Mirror of Bo: min at Pb."""
    if not pb or len(pressures) < 2:
        return True
    return True  # Simplified -- full check similar to Bo mirror
...
def run_exact_calculation(formula_key: str, kwargs: Dict[str, float]) -> str:
    ...
    if missing:
        return (
            f"Missing required inputs for {formula['name_en']}:\n"
            f"  Missing: {', '.join(sorted(missing))}\n"
            f"  Required: {', '.join(required_inputs)}\n"
...
def run_correlation(correlation_key: str, kwargs: Dict[str, float]) -> str:
    ...
    if missing:
        return (
            f"Missing required inputs for {corr['name_en']}:\n"
            f"  Missing: {', '.join(sorted(missing))}\n"
            f"  Required: {', '.join(required_inputs)}\n"
            f"  Units: {corr['units']}"
        )
    ...
    if range_warnings:
        output_lines.append("\nWARNING: Inputs outside correlation applicability range:")
