...
class AIService:
    ...
    def _build_engineering_context() -> str:
        ...
        context_parts.append(
            ...
            "for exact formulas; "
            "/calc ipr [model=auto|linear|vogel|composite] pr= pb= q_test= pwf_test= [pwf=] [plot=1] "
            "for IPR calculations (Linear PI, Vogel, Composite IPR) — NEVER answer IPR "
            "questions in prose; always route to this command; "
            "/calc vlp thp= tvd= id= q= gor= rs= api= gamma_g= mu_l= bo= t_wh= geothermal= [wc= q_w= gamma_w= bw= z= sigma= segments= plot=1] "
            "(single rate) or '/calc vlp ... q_min= q_max= plot=1' (calculated curve) for "
            "VLP — required BHP vs production rate using the Beggs-Brill (1973) segmented "
            "traverse — NEVER answer VLP questions in prose; always route to this command; "
            "/calc nodal [model=auto|linear|vogel|composite] pr= [pb=] [j=] [qmax=] [q_test= pwf_test=] "
            "thp= tvd= id= gor= rs= api= gamma_g= mu_l= bo= t_wh= geothermal= [wc= gamma_w= bw= z= "
            "sigma= segments= plot=1] for Nodal Analysis — the deterministic IPR–VLP operating-point "
            "solver (grid-scan + bracketed bisection) that returns the well's operating rate and flowing "
            "BHP together with the residual, the selected IPR model, and an optional calculated "
            "IPR+VLP overlay plot (plot=1) — NEVER answer operating-point questions in prose; always "
            "route to this command; "
            "/calc sensitivity type=thp|id|wc|gor thp=|id=|wc=|gor=<list of values> "
            "[thp_min= thp_max= n_points=] [plot=1] + the full IPR and VLP inputs of /calc nodal "
            "for Nodal Sensitivity Analysis — a deterministic sweep of THP, tubing ID, water cut or "
            "produced GOR around the operating point, reporting each scenario's operating rate, flowing "
            "BHP, and delta-q versus the base case — NEVER answer sensitivity questions in prose; "
            "always route to this command; "
            "/calc optimize type=thp|id|wc|gor id=|wc=|gor=|thp=<list of candidate values> "
            "objective=max_oil_rate [min_pwf= max_drawdown= max_liquid_rate= max_water_cut= min_thp= "
            "max_thp= max_gor=] + the full IPR and VLP inputs of /calc nodal "
            "for Production Optimization — candidate evaluation against the nodal operating point with "
            "engineering constraints, returning the best feasible candidate with deltas versus the "
            "current operating point — NEVER answer optimization questions in prose; always "
            "route to this command; "
