...
* In-situ injected gas volume is computed with the ideal-gas volume relation at
  ...
  transient unloading, multiphase VLP, or Black-Oil PVT correlation is used.
...
full VLP/Nodal coupling, or field operating instructions.  Increment 9 may
inject a PVT provider through the optional ``pvt_provider``/``pvt_context``
seam; Increment 8 rejects those arguments so Black-Oil integration cannot be
silently activated early.
"""
...
class GasLiftEngine:
    ...
    def calculate(
        ...
        *,
        ...
        ``pvt_provider`` and ``pvt_context`` are reserved for Increment 9.  The
        explicit rejection is intentional: Increment 8 must not integrate
        Black-Oil PVT or silently change the model by accepting those values.
        """
        if pvt_provider is not None or pvt_context is not None:
            raise GasLiftError(
                "INVALID_INPUT",
                "Black-Oil/PVT integration is reserved for Increment 9; "
                "Gas-Lift V1 accepts no PVT provider in this release.",
            )
        self._validate(inputs)

        temp_r = inputs.average_temperature_f + 459.67
        gas_k = self.GAS_CONSTANT * inputs.gas_specific_gravity / (
            inputs.z_factor * temp_r
        )
        ...
        liquid_in_situ_bpd = (
            oil_surface * inputs.oil_fvf_rb_stb
