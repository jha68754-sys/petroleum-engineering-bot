...
def test_offset_defaults_to_railway_volume_when_available(tmp_path, monkeypatch):
    volume_path = tmp_path / "mounted-volume"
    monkeypatch.delenv("OFFSET_STATE_FILE", raising=False)
    monkeypatch.setenv("RAILWAY_VOLUME_MOUNT_PATH", str(volume_path))
    config_globals = runpy.run_path(str(Path(__file__).parents[1] / "config.py"))

    assert Path(config_globals["OFFSET_STATE_FILE"]) == volume_path / "offset_state.json"
