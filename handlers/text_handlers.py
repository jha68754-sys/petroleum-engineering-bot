...
/cmg, /report, /glossary, /reset, /start, /analyze, /graph.
...
def handle_start(message: Dict[str, Any], tg) -> Tuple[str, Optional[bytes], Optional[str]]:
    ...
    return START_MESSAGE, None, None
...
def handle_reset(message: Dict[str, Any], tg) -> Tuple[str, Optional[bytes], Optional[str]]:
    ...
    from main import FILE_CONTEXT, IMAGE_CONTEXT
...
def handle_classify(message: Dict[str, Any], tg) -> Tuple[str, Optional[bytes], Optional[str]]:
    """Handle /classify gor=<val> api=<val> command."""
...
@registry.register("graph", aliases=["image", "photo"])
def handle_graph(message: Dict[str, Any], tg) -> Tuple[str, Optional[bytes], Optional[str]]:
    """Handle /graph command."""
    chat_id = message.get("chat", {}).get("id", "")
    from main import IMAGE_CONTEXT
    image_path = IMAGE_CONTEXT.get(chat_id)
    if not image_path:
        return "No image uploaded. Upload a PNG/JPG chart first.", None, None
    return None, None, None  # Signal that AI vision analysis is needed
