...
START_MESSAGE = (
    ...
    "مساعد هندسي للحسابات النفطية الحتمية والتحليل المساند.\n\n"
    "الحسابات الرقمية تستخدم محركات محددة وتعرض المدخلات والوحدات والقيود.\n"
    "النتائج حسابات نموذجية وليست قياسات حقلية أو تعليمات تشغيلية.\n\n"
    "لرؤية الأوامر المدعومة استخدم /help.\n"
    "يمكنك أن تسأل طبيعيًا: ما معنى Bo؟ أو What is Pwf? أو الفرق بين Bo و Bg؟\n"
    "الأسئلة الرقمية يجب أن تستخدم الأمر الهندسي المناسب."
)

HELP_MESSAGE = (
    "Petroleum Engineering Bot — الأوامر المدعومة\n\n"
    "الحسابات الحتمية:\n"
    "/calc <type> key=value ...\n"
    "  يدعم الحسابات المسجلة مثل ooip وogip وdarcy وvogel وnpv وgas_lift وchoke.\n"
    "  مثال: /calc ooip area=500 h=50 phi=0.2 sw=0.3 bo=1.3\n\n"
    "/calc vlp ...\n"
    "  حساب VLP بطريقة Beggs-Brill عند توفير جميع المدخلات المطلوبة.\n\n"
    "/calc nodal ...\n"
    "  حل IPR وVLP لنقطة التشغيل باستخدام model=auto أو linear أو vogel أو composite.\n\n"
    "/calc sensitivity ...\n"
    "  تحليل حساسية deterministic لمتغيرات THP أو tubing ID أو water cut أو GOR.\n\n"
    "/calc optimize ...\n"
    "  مقارنة مرشحين ضمن objective=max_oil_rate والقيود الصريحة المقدمة.\n\n"
    "/calc compare type=choke|system scenario=<label>:key=value scenario=<label>:key=value ...\n"
    "  مقارنة حتمية بين سيناريوهين أو أكثر فوق محرك Choke أو System؛ يدعم overrides صريحة فقط.\n"
    "  استخدم /comparison report|replay|json <comparison_id> للتتبع وإعادة التشغيل.\n\n"
    "/calc system ...\n"
    "  حل نقطة التشغيل المتكاملة بين IPR وVLP وGilbert choke.\n"
    "  استخدم case=1 لحفظ Engineering Case قابل للتقرير وإعادة التشغيل.\n\n"
    "/case report <case_id>\n"
    ...
    "/case restore\n"
    "  استعادة Snapshot مرفوع إلى نفس Case registry.\n"
    "/case resume\n"
    "  استعادة Snapshot وإعادة الحساب الحتمي؛ يمكن تحديد THP صراحةً بوحدة psia.\n"
    "/case json <case_id>\n"
    "  عرض الحالة بصيغة JSON.\n\n"
    "/define <term>\n"
    "  تعريف مصطلح نفطي بالعربية والإنجليزية مع الوحدة والعلاقات والسياق.\n"
    "  مثال: /define Bo أو /define Pwf. ويمكنك أيضًا السؤال دون أمر.\n\n"
    "أوامر PVT والتحقق:\n"
    "/classify gor=<scf/STB> api=<deg API> [no_liquid=1]\n"
    "/estimate <type> key=value ...\n"
    "/check <relationship> p=p1,p2 v=v1,v2 [pb=...]\n"
    "/plot <type> p=x1,x2 v=y1,y2 [pb=...]\n"
    "  الرسم يعتمد على القيم التي يرسلها المستخدم فقط ولا يطلب رفع مستند.\n"
    "/convert <value> <from> to <to>\n\n"
    "الملفات والأسئلة العامة:\n"
    "/analyze — بعد رفع PDF أو DOCX أو Excel أو CSV لتحليل النص المستخرج.\n"
    "/analyze well <وصف المشكلة> — استخراج القيم الهندسية المذكورة صراحةً قبل الحساب.\n"
    "  ويمكن إرسال /analyze well أولًا ثم إرسال وصف المشكلة في الرسالة التالية.\n"
    "/report — تقرير PVT بعد رفع سياق أو مستند؛ بدونه سيطلب البوت البيانات اللازمة.\n"
    "/reset — مسح سياق الملف والصورة للمحادثة الحالية.\n"
    "/surface_separator — إرشادات إعادة تركيب عينات الزيت والغاز السطحية.\n"
    "/pvto /pvdo /pvtg /pvdg — إرشادات تنسيق جداول المحاكاة، وليست مولدًا لبيانات مختبرية.\n"
    "/export_sim — قرار إرشادي حول نوع جدول المحاكاة، وليس تصدير ملف محاكاة.\n"
    "/eclipse /cmg — معلومات إرشادية عن اختيار simulator.\n"
    "الأسئلة العامة تُجاب عبر AI للشرح فقط؛ لا ينفذ AI الحسابات الحتمية ولا يعوض البيانات المفقودة.\n"
    "لا هلوسة ولا اختلاق لنتائج رقمية أو ملفات أو قياسات غير موجودة.\n\n"
    "حدود مهمة: لا ينشئ البوت قياسات مفقودة، ولا يختلق PVT، ولا يحول نتيجة النموذج إلى توصية تشغيلية. "
    "عند نقص البيانات أو عدم صلاحية الحالة، يعرض رسالة خطأ typed ويطلب المدخلات المطلوبة."
)
